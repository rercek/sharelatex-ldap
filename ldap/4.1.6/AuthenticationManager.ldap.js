...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
  ...
  createIfNotExistAndLogin(
    ...
    if (!user) {
	  ...
	  console.log("betaProgram flag set to true to specify a ldap user in the mongodb");
  ...
  async ldapAuth(
    ...
    try {
      ...
      //add this test because the LDAP_USER_FILTER should only find one user!
      if (searchEntries.length > 1) {
          console.log("Check LDAP_USER_FILTER, " + searchEntries.length + " users were found in the LDAP, but only one should have been!")
      } else if (searchEntries[0]) {
    ...
    User.findOne(query, (error, user) => {
      ...
      if (user && user.hashedPassword) {
        //console.log('******************** LOGIN ******************')
        // check if the flag betaProgram corresponding to ldap auth is false, so update the flag 
        if (user.betaProgram == false) {
            user.betaProgram = true
            user.save()
        }
        AuthenticationManager.login(user, "randomPass", callback)
